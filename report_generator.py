...
def generate_report_content(transcript, manual_text, website_text, sales_material_text, model_client):
    ...
    4. 全体統括コメントについて、商談の文字起こしを深く分析し、営業担当者のための「全体統括コメント」を1000字程度で作成してください。

        評価の必須観点
        単なる表面的な会話の要約や「ヒアリングが丁寧でした」といった浅い感想は絶対に禁止します。以下の3つの観点から、具体的かつ戦略的なフィードバックを行ってください。
        1. 営業スタイルと優位性 (Good)
        受身的・御用聞きの営業になっていないか
        顕在ニーズだけでなく、潜在的ニーズに対するアプローチができているか
        他社（他顧問サービス等）と比較した際の強みや優位性は商談内で出せていたか
        
        2. 顧客の「納得度」と価格提示の戦略 (More)
        料金（特に初期費用などの高額なハードル）に対する顧客の納得度はどうだったか
        料金提示のタイミングは適切だったか（早い段階で提示してしまい、期待値のハードルを無駄に上げていないか等）
        費用対効果をどう説明すべきだったか

        3. サービスの訴求ポイントと次回へのアドバイス (More & Next Steps)
        SaaSのような分かりやすい商材と違い、無形商材（人材、顧問等）特有の「本当に動いて結果を出してくれるか分からない」という不安を払拭できていたか
        顧客が直観的にメリットを感じられるように、どのような数値や事例を用いるべきだったか
        次回の商談やクロージングに向けて、具体的にどんなアクションやトーク、資料を追加するべきか

        出力ルール
        評価者としての客観的でプロフェッショナルなトーンで記述すること
        実際の文字起こし内の発言や文脈を根拠（具体例）として交えながら語ること
        良かった点、改善点、次への具体的なアドバイスが、箇条書きの羅列ではなく自然な文章の流れ（読み物）として読めるように構成すること

    5. 質疑応答（Q&A）は、商談内で実際に行われたやり取りを抽出してください。その際、内容をそのまま記載するのではなく、質問と回答の要点を簡潔にまとめてください。
