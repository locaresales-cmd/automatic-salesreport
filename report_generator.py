...
def generate_report_content(transcript, manual_text, website_text, sales_material_text, model_client):
    ...
    prompt_template = """
    あなたは超一流のBtoBセールス・イネーブルメント（営業組織強化）の専門家です。ですが、今回は「営業を受ける側（発注側）」の視点でレポートを整理します。
    ...
    4. 全体統括コメントについて、提供する商談の文字起こしを深く分析し、営業担当者のための「全体統括コメント」を【600〜800字程度】で簡潔に作成してください。

        評価の必須観点
        単なる会話の要約や浅い感想は禁止します。以下の3つの観点から、具体的かつ戦略的なフィードバックを端的に行ってください。

        営業スタイルと優位性 (Good/More)
        受身的になっていないか。顕在・潜在ニーズへのアプローチと、他社と比較した際の強み（優位性）は商談内で伝わっていたか。

        顧客の「納得度」と価格提示の戦略 (More)
        料金（初期費用など）に対する顧客の納得度はどうだったか。価格提示のタイミングや、費用対効果のロジックは適切だったか。

        サービスの訴求ポイントと次回へのアドバイス (More ＆ Next Steps)
        無形商材特有の不安を払拭できていたか。次回に向けて、具体的にどんなアクション、トーク、数値を交えた資料を追加すべきか。

        出力ルール
        前置きや冗長な背景説明は一切省き、結論ファーストで記述すること。
        全体で600〜800文字程度に収めること。

        【重要】出力テキスト内にアスタリスク記号は絶対に使用しないこと。見出しや強調したい箇所には【】や「」を使用すること。
        実際の文字起こし内の発言を根拠として端的に交えつつ、読みやすい文章で構成すること。
