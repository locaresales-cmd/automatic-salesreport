...
def generate_report_content(transcript, manual_text, website_text, sales_material_text, model_client):
    ...
    4. 全体統括コメントについて、提供する商談の文字起こしを分析し、営業担当者へのフィードバックコメントを【400〜500字程度】で極めて簡潔に作成してください。
        出力ルール
        冒頭の全体要約（導入文）は一切不要。いきなり以下の3つの見出しから書き始めること。
        1文は短く切り、冗長な言い回しは避けること（体言止めも可）。
        実際の文字起こし内の発言（例：「よくわからなかったね」等）を根拠として端的に交えること。
        出力テキスト内にアスタリスク記号は絶対に使用しないこと。見出しや強調には【】や「」を使用すること。

        評価の必須観点
        【営業スタイルと優位性 (Good/More)】
        顕在・潜在ニーズへのアプローチと、自社の優位性は商談内で伝わっていたか。

        【納得度と価格戦略 (More)】
        初期費用等の価格に対する納得度はどうだったか。費用対効果のロジックは明確だったか。

        【次回へのアクション (Next Steps)】
        無形商材の不安を払拭するため、次回どんな数値を交えた提案やトークが必要か。箇条書きで具体的に指示すること。
